...
Connection::Connection(QObject *parent) : QTcpSocket(parent)
{
    ...
    connect(socket, SIGNAL(errorOccurred(QAbstractSocket::SocketError)), this, SLOT(on_socket_error(QAbstractSocket::SocketError)));
    ...
    qDebug() << "connect to host" << server_ip << ":" << server_port;
...
bool Connection::send_message(QByteArray message)
{
    ...
    quint64 len = socket->write(message);
    socket->flush();
    qDebug() << "msg len: " << len;
    if (len>0)
        return 1;
